    ...
    describe("relaxed compatibility check:", function()
      ...
      local allowed_cases = {
        ...
        ["CP and DP patch version mismatches"] = {
          dp_version = string.format("%d.%d.%d", MAJOR, MINOR, PATCH + 1),
        },
      ...
      for i, p in ipairs(pl2) do
        ...
        -- find a plugin that has minor larger than 2 :joy:
        ...
        if minor and tonumber(minor) and tonumber(minor) > 2 then
          pl2[i].version = string.format("%d.%d.%d",
            tonumber(v:match("(%d+)")),
            tonumber(v:match("%d+%.(%d+)")) - 2,
            tonumber(v:match("%d+%.%d+%.(%d+)"))
          )
          break
        end
      end
      allowed_cases["0 <= CP.plugin.minor - DP.plugin.minor <= 2"] = {
        plugins_list = pl2
      }
      ...
      -- desc = { dp_kong_version_override, cp_kong_version_override, plugin_list_override, expected_status }
      local blocked_cases = {
      ...
      for i, p in ipairs(pl1) do
        ...
        local minor = v:match("%d+%.(%d+)")
        -- find a plugin that has minor larger than 3 :joy:
        -- we hardcode `dummy` plugin to be 9.9.9 so there must be at least one
        if minor and tonumber(minor) and tonumber(minor) > 3 then
          pl1[i].version = string.format("%d.%d.%d",
            tonumber(v:match("(%d+)")),
            tonumber(v:match("%d+%.(%d+)")) - 3,
            tonumber(v:match("%d+%.%d+%.(%d+)"))
          )
          break
        end
      end
      blocked_cases["CP.plugin.minor - DP.plugin.minor > 2"] = {
        plugins_list = pl1,
        expected = CLUSTERING_SYNC_STATUS.PLUGIN_VERSION_INCOMPATIBLE,
      }

      local pl2 = pl_tablex.deepcopy(helpers.get_plugins_list())
      local pv2 = pl2[1].version
      pl2[1].version = string.format("%d.%d.%d",
        tonumber(pv2:match("(%d+)")),
        tonumber(pv2:match("%d+%.(%d+)")) + 1,
        tonumber(pv2:match("%d+%.%d+%.(%d+)"))
      )
      blocked_cases["CP.plugin.minor - DP.plugin.minor < 0"] = {
        plugins_list = pl2,
        expected = CLUSTERING_SYNC_STATUS.PLUGIN_VERSION_INCOMPATIBLE,
      }
